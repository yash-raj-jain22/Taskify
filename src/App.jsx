...
function App() {
  ...
  return (
    <>
      <Navbar />
      <div className="main">
        <div className="container mx-auto p-4 mt-20 bg-indigo-100 rounded-lg min-h-120">
          <div className="heading flex flex-col justify-center items-center mb-4">
            <h1 className="text-4xl font-bold mb-10 underline">Taskify- Manage all your tasks</h1>
            <h1 className="text-3xl font-bold">ToDo List</h1>
          </div>
          <div className="input flex justify-between items-center mb-4">
            <input type="text" onChange={handleChange} value={Todo} placeholder='Enter a new task...' className='h-20 bg-slate-50 border-t-2 border-l-2 border-b-2  rounded-l-2xl p-2 focus:outline-none  focus:border-black focus:bg-white border-blue-500  w-full ' />
            <button onClick={handleAddTask}   className={`font-bold  text-white px-4 py-2 mr-10 h-20 w-50 cursor-pointer rounded-r-2xl  ${Todo.length==0?"bg-blue-400 hover:bg-blue-400 hover:cursor-not-allowed":"bg-blue-500 hover:bg-blue-600"}`}>Add Task</button>
          </div>
          ...
          <div className="separator border-b-2 border-indigo-300 mb-8"></div>
                ...
                return((showFinished || !item.isCompleted)) && (
                  ...
                  <li key={Todos.indexOf(item)} className="todo p-6 border-2 rounded-2xl mb-5 flex justify-between items-center flex-col md:flex-row gap-3">
                    <div className="todoText flex items-center justify-center">
                      ...
                      <span className={item.isCompleted ? "text-xl line-through" : "text-xl"}>{item.todo}</span>
                    </div>
